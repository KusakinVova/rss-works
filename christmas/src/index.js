...
//   toys.forEach((el, i) => {
//     toyOn.innerHTML += `
//       <img class="toy-on-tree" src="../assets/toys/${el.numberToy}.png"
//       data-num="${el.numberToy}" data-numi="${i}" alt="pict"
//       draggable="true" style="top: ${el.position[1]}px; left: ${el.position[0]}px;">
//       `;
//   });
// }
...
if (tree) {
  tree.addEventListener('drop', (el) => {
    console.log(el);
    const target = el;
    const outToy = target.dataTransfer?.getData('out');

    if (outToy == 'true') {
      const id = target.dataTransfer.getData('id');
      if (id && +data[+id - 1].count > 0) {
        toys.push({
        ...
        const oldCount = (+Module.data[+id - 1].count);
        Module.data[+id - 1].count = oldCount - 1;

      }
    } else if (outToy == 'false') {
      toys[+target.dataTransfer.getData('id')].position = [+`${target.layerX}` - +target.dataTransfer.getData('offsetX'), +`${target.layerY}` - +target.dataTransfer.getData('offsetY')];
    }
    View.renderTreePage();
  });
}
